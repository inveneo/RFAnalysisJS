...
function setElevation(geoJson,elevJson)
{
  ...
  return geoJson;
    ...
    // var i,j,temparray,chunk = 400;
    // for (i=0,j=latLngCoords.length; i<j; i+=chunk) {
    //     temparray = latLngCoords.slice(i,i+chunk);
        
    //     // Create a LocationElevationRequest object using the array's one value
    //     var positionalRequest = {'locations':temparray};
    //     // Initiate the location request
    //     elevator.getElevationForLocations(positionalRequest, function(results, status) {
    //       if (status == google.maps.ElevationStatus.OK) 
    //       {
    //         elevationArray.push(results);
    //         console.log(elevationArray);
    //       }
    //     });
    //   }
